...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ОбрабочикОповещенияВопросСохранитьИзменения", ЭтаФорма);
...
&НаКлиенте
Процедура ОбрабочикОповещенияВопросСохранитьИзменения(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если Не РезультатВопроса = Неопределено И РезультатВопроса = КодВозвратаДиалога.Да Тогда	
		
		ДиалогСохраненияФайла(Объект.ИмяФайла, Истина);
...
Процедура ТипВФормеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОповещениеОВыборе = Новый ОписаниеОповещения("ОбработчикВыбораТипа", ЭтаФорма);
	СписокТипов.ПоказатьВыборЭлемента(ОповещениеОВыборе, ЗаголовокЭлемента);
	
КонецПроцедуры

&НаКлиенте
Процедура ОбработчикВыбораТипа(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
...
Процедура ОткрытьКонструкторЗапроса(Команда)
	...
	#Иначе
		...
		СообщениеПользователю.Текст = НСтр("ru = 'Конструктор запроса доступен только в режиме Толстого клиента.'");
...
Процедура СохранитьЗапросыВФайл(Команда)
	...
	//	Модифицированность	= Ложь;
	//КонецЕсли;	
	ДиалогСохраненияФайла(ИмяФайла);
...
//
&НаКлиенте
Процедура ДиалогСохраненияФайла(ИмяФайла, ЗакрытьОкно = Ложь)
	...
	Если РасширениеПодключено Тогда 
		...
		Если ПустаяСтрока(ИмяФайла) Тогда 
			...
			ДополнительныеПараметры.Вставить("ЗакрытьОкно", ЗакрытьОкно);
			ОписаниеОповещения = Новый ОписаниеОповещения("ОбработчикОповещенияСохраненияЗапроса",ЭтаФорма, ДополнительныеПараметры);
			Диалог.Показать(ОписаниеОповещения);
			
		Иначе
			
			ВыбранныеФайлы = Новый Массив;
			ВыбранныеФайлы.Добавить(ИмяФайла);
			ОбработчикОповещенияСохраненияЗапроса(ВыбранныеФайлы);
...
&НаКлиенте
Процедура ОбработчикОповещенияСохраненияЗапроса(ВыбранныеФайлы, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если Не ДополнительныеПараметры = Неопределено И ДополнительныеПараметры.ЗакрытьОкно = Истина Тогда
		Закрыть();
	КонецЕсли;
...
Процедура ОбработкаЧтенияФайла(Удалять)
	...
	Если РасширениеПодключено Тогда
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Удалять", Удалять);
		ОписаниеОповещения = Новый ОписаниеОповещения("ОбработчикОповещенияОткрытияЗапроса",ЭтаФорма, ДополнительныеПараметры);
		Диалог.Показать(ОписаниеОповещения);
...
&НаКлиенте
Процедура ОбработчикОповещенияОткрытияЗапроса(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
