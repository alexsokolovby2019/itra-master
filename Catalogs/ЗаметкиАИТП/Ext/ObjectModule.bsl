...
Процедура ПередЗаписью(Отказ)
	...
	Если ЗначениеЗаполнено(Родитель) И Родитель.Автор <> Автор Тогда
		ОбщегоНазначенияКлиентСерверАИТП.СообщитьПользователю(НСтр("ru = 'Нельзя указывать группу другого пользователя.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли;
	...
	Если Не ЭтоГруппа Тогда 
		...
		ПредставлениеПредмета = ОбщегоНазначенияАИТП.ПредметСтрокой(Предмет);
