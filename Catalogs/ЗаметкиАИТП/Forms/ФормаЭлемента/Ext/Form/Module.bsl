...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Предмет") Тогда 
		...
		Объект.ПредставлениеПредмета = ОбщегоНазначенияАИТП.ПредметСтрокой(Объект.Предмет);
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.ТекстСодержания = СтроковыеФункцииКлиентСерверАИТП.ИзвлечьТекстИзHTML(ТекстHTML);
...
Процедура УстановитьВидимость()
	...
	ОткрытаАвтором = Объект.Автор = ПользователиАИТП.ТекущийПользователь();
